...
export default async function handler(
    ...
    try {
        ...
        if (!serverId) {
            return res.status(400).json({ error: "Thông tin danh sách thành viên bị thiếu" });
        }

        if (!channelId) {
            return res.status(400).json({ error: "Thông tin lớp học bị thiếu" });
        }
        ...
        if (!server) {
            return res.status(404).json({ error: "Không tìm thấy danh sách thành viên" });
        }
        ...
        if (!member) {
            return res.status(404).json({ error: "Thành viên chưa tham gia lớp" });
        }
