...
export const EssayExerciseTakingPage = ({ 
    ...
    try {
      ...
      if (exerciseData?.deadline) {
        ...
        if (currentDate > deadlineDate) {
          ...
          const minutesLate = Math.floor((currentDate.getTime() - deadlineDate.getTime()) / (1000 * 60));
          
          // Apply penalty based on minutes late
          if (minutesLate > 0) {
            if (minutesLate <= 30) {
              // 1-30 minutes: -0.5 points
              latePenalty = {
                amount: 0.5,
                reason: "Nộp muộn trong vòng 30 phút (-0.5 điểm)",
              ...
              latePenalty = {
                amount: 2,
                reason: "Nộp muộn từ 31-60 phút (-2 điểm)",
                minutesLate: minutesLate
              };
            } else {
              // Over 60 minutes: -50% points
              latePenalty = {
                amount: -1, // Special value to indicate percentage-based penalty
                reason: "Nộp muộn trên 1 giờ (trừ 50% tổng điểm)",
                minutesLate: minutesLate
              };
            }
            
            console.log(`Late submission detected: ${minutesLate} minutes late. Applying penalty:`, latePenalty);
          }
        }
      }
  ...
  const formatDeadline = (deadline: string | null | undefined): { text: string, isOverdue: boolean } | null => {
    ...
    try {
      ...
      const formattedDate = deadlineDate.toLocaleDateString('vi-VN', options).replace(',', ' -');
  ...
  // Check if late submission
  const checkLateSubmission = (): { isLate: boolean, minutesLate?: number } => {
    if (!exerciseData?.deadline) return { isLate: false };
    ...
    try {
      ...
      if (now > deadlineDate) {
        const minutesLate = Math.floor((now.getTime() - deadlineDate.getTime()) / (1000 * 60));
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <Card className="max-w-3xl mx-auto">
            ...
            <CardContent className="flex flex-col items-center justify-center">
              ...
              <p className="text-lg font-medium mb-2">Bạn đã hoàn thành bài tập này</p>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {exerciseData?.deadline && (
        <div className="bg-gray-50 dark:bg-gray-800/50 py-1 px-4 text-xs border-b flex items-center justify-center">
          <Clock className="h-3 w-3 mr-1" />
          <span className="mr-1">Hạn nộp:</span>
          {(() => {
            const deadline = formatDeadline(exerciseData.deadline);
            if (!deadline) return null;
            
            const lateStatus = checkLateSubmission();
            if (lateStatus.isLate) {
              return (
                <span className="font-medium text-red-500 dark:text-red-400">
                  {deadline.text} (Đã hết hạn - Muộn {lateStatus.minutesLate} phút)
                </span>
              );
            }
