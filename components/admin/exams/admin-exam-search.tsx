...
interface AdminExamSearchProps {
  data: {
    label: string;
...
export const AdminExamSearch = ({
  data
}: AdminExamSearchProps) => {
  ...
  const onClick = ({ id, type }: { id: string, type: string }) => {
    ...
    if (type === "all") {
      return router.push(`/admin/exams?view=all`);
    }

    if (type === "server") {
      return router.push(`/admin/exams?serverId=${id}`);
    }
    ...
    if (type === "channel") {
      ...
      if (serverId) {
        return router.push(`/admin/exams?serverId=${serverId}&channelId=${id}`);
      } else {
        return router.push(`/admin/exams?channelId=${id}`);
      }
    }
  }
